...
_snapshots = {}
_quarter_fingerprint = None


def _check_quarter_set(quarters):
    """
    Drop cached snapshots when the set of available quarters changes.

    Without this, uploading a new workbook leaves the previously-cached
    snapshots in place — the ledger picks up the new quarter but the q/q
    comparison can still be built from scores computed before the upload.
    """
    global _quarter_fingerprint
    fp = tuple(quarters or ())
    if fp != _quarter_fingerprint:
        _quarter_fingerprint = fp
        _snapshots.clear()
...
def render_home():
    ...
    _check_quarter_set(quarters)
    as_of = quarters[0] if quarters else None
    # The detected list is the authority on what the previous quarter IS —
    # inferring it from the label assumes an unbroken sequence.
    prior_q = quarters[1] if len(quarters) > 1 else (_quarter_before(as_of) if as_of else None)
    ...
    return render_template(
        ...
        # The header's two big numbers disagree on purpose — these let the page
        # show its work rather than leaving the gap unexplained.
        eligible_total=len(scored_keys),
        no_book=no_book,
        filters_active=(rep_filter != "__all__" or band != "__all__" or bool(q_text)),
...
def _pdf_engine_name():
    ...
    ignores CSS @page margin boxes and takes its running bands from the print
    API instead — emitting both would double them.
    """
    ok, _e = _chromium_ok()
    if ok:
        return "chromium"
...
# Font stacks kept as their own constants: the CSS needs single quotes around
# the family name, and the surrounding HTML attribute uses double quotes, so
# inlining them is where the escaping goes wrong.
_HDR_SANS = "'IBM Plex Sans',Segoe UI,sans-serif"
_HDR_MONO = "'IBM Plex Mono',Consolas,monospace"
...
def _running_header(facts, logo):
    ...
    Rendered in isolation — no page stylesheet, no custom properties, no
    external CSS — so every value is inline and literal, and lengths are px
    because Chrome lays this box out at 96dpi.
    """
    if logo:
        brand = '<img src="' + logo + '" style="width:118px;height:auto;display:block;">'
    else:
        brand = ('<div style="font-size:14px;font-weight:700;">SouthState'
    ...
    return (
        ...
        'font-family:' + _HDR_SANS + ';">'
        + brand +
        '<div style="text-align:right;">'
        '<div style="font-size:12px;font-weight:600;letter-spacing:-.01em;">'
        + str(facts.get("bank", "")) +
        '</div>'
        ...
        '<b style="color:#C9A227;font-weight:600;">'
        + str(facts.get("as_of", "")) +
        '</b></div></div></div></div>'
    )
...
def _running_footer(facts):
    """Disclaimer and page counter, repeated on every sheet."""
    total = facts.get("universe_total") or 0
    return (
        '<div style="width:100%;padding:0 48px;box-sizing:border-box;'
        'font-family:' + _HDR_SANS + ';font-size:7px;line-height:1.5;color:#54687F;">'
        '<div style="border-top:1px solid #D3DBE5;padding-top:6px;'
        'display:flex;align-items:flex-start;gap:14px;">'
        '<div style="flex:1;text-align:left;">'
        '<b>Internal analytical aid &mdash; not investment advice or a solicitation.</b> '
        'Any idea should fit the institution&rsquo;s objectives and pass suitability '
        'review before action. Figures are SNL / Capital IQ call-report data, traceable '
        'to the FFIEC filing via the identifiers on page 1. Percentiles are ranked '
        'against all ' + format(total, ",") + ' depositories filing for '
        + str(facts.get("as_of", "")) + '. For internal SouthState Securities use.'
        '</div>'
        '<div style="flex:0 0 auto;font-family:' + _HDR_MONO + ';font-size:7px;'
        'font-weight:600;color:#7488A0;white-space:nowrap;padding-top:1px;">'
...
#
# A cohort screen is the same ledger as Bank Universe, scoped to the banks the
# desk picked. It is deliberately NOT a second table design: same columns, same
# heat ramp, same row target. The old results page also carried a per-bank
# rationale accordion and PDF buttons — both now live on the detail page, so
# repeating them here would be two routes to the same place.
# ==========================================================================
...
def render_results():
    ...
    picked = request.form.getlist("banks")
    quarters_sel = request.form.getlist("quarters")
    rep_filter = request.form.get("rep_filter", "__all__")

    _options, all_quarters = analysis.get_available_options()
    all_quarters = sorted(all_quarters or [], key=qkey, reverse=True)
    quarters_sel = sorted(quarters_sel or [], key=qkey, reverse=True)
    as_of = quarters_sel[0] if quarters_sel else (all_quarters[0] if all_quarters else None)
    prior_q = quarters_sel[1] if len(quarters_sel) > 1 else _quarter_before(as_of)

    rows = []
    if picked and as_of:
        rows = analysis.run_analysis(
            banks=picked, quarters=[as_of], rep_filter=rep_filter
        ) or []

    now, universe_total = _snapshot(as_of) if as_of else ({}, 0)
    was, _t = _snapshot(prior_q) if prior_q else ({}, 0)

    meta = {}
    try:
        opts, _q = analysis.get_available_options()
        for o in opts or []:
            if o.get("key"):
                meta[o["key"]] = o
    except Exception:
    ...
    # Sparkline history, oldest first — same series the ledger draws.
    hist = []
    if SPARK_QUARTERS > 1 and all_quarters and as_of in all_quarters:
        start = all_quarters.index(as_of)
        hist = list(reversed(all_quarters[start:start + SPARK_QUARTERS]))
    snaps = [_snapshot(q)[0] for q in hist] if hist else []

    for r in rows:
        k = r.get("key")
        w = was.get(k)
        pc = w.get("composite") if w else None
        cc = r.get("composite")
        r["delta"] = round(cc - pc) if (pc is not None and cc is not None) else None
        r["assets_fmt"] = (meta.get(k) or {}).get("assets_fmt")
        if not r.get("location"):
            r["location"] = (now.get(k) or {}).get("location") or ""
        if snaps:
            r["spark"] = _spark_points([s.get(k, {}).get("composite") for s in snaps])

    rows.sort(key=lambda r: (r.get("composite") is None, -(r.get("composite") or 0)))

    above = sum(1 for r in rows if (r.get("composite") or 0) >= threshold)
    rep_label = "All reps"
    try:
        for o in reps.rep_options():
            if o.get("value") == rep_filter:
                rep_label = o.get("label")
                break
    except Exception:
        pass
