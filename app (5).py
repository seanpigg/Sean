...
def analyze():
    """
    Custom screen results — the ledger scoped to a chosen cohort.

    The shaping lives in desk.render_results() so this page picks up the same
    deltas, sparklines and asset formatting the Bank Universe ledger uses.
    """
    if not request.form.getlist("banks") or not request.form.getlist("quarters"):
        flash("Pick at least one bank and one quarter.")
        return redirect(url_for("home"))
    return desk.render_results()
...
def pdf():
    ...
    # No approved narrative to carry: only generate one if the caller asked for
    # it. A bare /pdf link exports the figures, which is the whole point of
    # making the model call explicit on the detail page.
    if narrative is None and request.args.get("narrative") == "1":
        try:
    ...
    # Preferred path: render templates/pdf.html, so the brief and the app share
    # one design. Returns None if WeasyPrint is not installed.
    pdf_bytes = desk.build_call_pdf(bank, as_of, voice=voice, narrative_data=narrative)
    if pdf_bytes:
    ...
    # Fallback: the original generator.
...
@app.route("/pdf-check")
def pdf_check():
    """Is the redesigned PDF renderer actually available? Reports why not."""
    import os
    status = desk.pdf_engine_status()
    tpl = os.path.join(app.root_path, "templates", "pdf.html")
    status["template_present"] = os.path.exists(tpl)
    status["last_render_error"] = desk.PDF_ENGINE_ERROR
    return status
