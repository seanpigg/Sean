...
def pdf_engine_status():
    """Whether the HTML renderer is usable, and why not if it isn't."""
    try:
        import weasyprint
        return {
            "ok": True,
            ...
            "note": "The redesigned PDF is in use.",
        }
    except Exception as e:
        return {
            "ok": False,
            ...
            "note": ("WeasyPrint could not be imported, so /pdf is falling back to the "
                     "original generator. On Windows this is almost always the missing "
                     "GTK3 runtime — pip installs the package but the native libraries "
...
def build_call_pdf(bank_key, as_of, voice=None, narrative_data=None, model=None):
    ...
    try:
        ...
        PDF_ENGINE_ERROR = "%s: %s" % (type(e).__name__, e)
        return None
